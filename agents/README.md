...
Agent是系统的**核心智能组件**，负责理解用户意图、自主决策工具调用并返回结果。本模块采用**混合架构**：

- **Orchestrator（编排器）**：负责意图识别和路由分发，简单对话直接走 LLM
- **ReAct 循环**：复杂任务（检索/诊断/指引）由子 Agent 内部使用 Thought → Action → Observation 循环，LLM 自主决定每步调用什么工具、何时追问用户、何时结束

架构核心思路：简单场景快速响应，复杂场景深度推理。
...
│          ├── CHAT ──► 直接 LLM 对话（快速响应）                 │
│          │                                                      │
│          ├── RETRIEVAL ──► RetrievalAgent (ReAct)               │
│          │     ┌─ ReAct 循环 ────────────────────┐              │
│          │     │ Think → 检索 → 评估 → 追问/返回 │              │
│          │     └─────────────────────────────────┘              │
│          │                                                      │
│          ├── DIAGNOSIS ──► DiagnosisAgent (ReAct)               │
│          │     ┌─ ReAct 循环 ────────────────────────────┐      │
│          │     │ Think → 检索 → 追问 → 推理 → 诊断       │      │
│          │     └─────────────────────────────────────────┘      │
│          │                                                      │
│          ├── GUIDANCE ──► GuidanceAgent (ReAct)                 │
│          │     ┌─ ReAct 循环 ───────────────────────────┐       │
│          │     │ Think → 检索流程 → 生成步骤 → 校验     │       │
│          │     └────────────────────────────────────────┘       │
│          │                                                      │
...
| `retrieval_agent` | retrieval_agent.py | 知识检索 | ReAct循环：检索→评估→追问→整理答案 |
| `diagnosis_agent` | diagnosis_agent.py | 故障诊断 | ReAct循环：检索→追问→推理→诊断排序 |
| `guidance_agent` | guidance_agent.py | 作业指引 | ReAct循环：检索流程→生成步骤→安全校验 |
| `memory_agent` | memory_agent.py | 记忆整理 | function calling：提取事实→冲突检测→向量存储 |
...
├── diagnosis_agent.py          # 诊断Agent
├── guidance_agent.py          # 作业Agent
└── memory_agent.py            # 记忆整理Agent
                                 #   依赖: services/llm_service.py (function calling)
                                 #         tools/fact_retrieval_tool.py (事实检索工具)
                                 #         services/vector_service.py (事实向量存储)
                                 #         embeddings/text_embedding.py (事实向量化)
```
