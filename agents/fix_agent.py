...
采用向量语义匹配 + 5分支策略，返回每条路径的匹配分数和历史故障标记。
- 适用：需要分析设备故障的因果关系、查找已知解决方案
- 参数：
  - keyword（设备名称关键字，可选）
  - component_description（部件描述，用于向量匹配部件，可选）
  - fault_description（故障描述，用于向量匹配故障，可选）
  - page（页码，默认0）、size（每页数量，默认5）
- 使用策略：从用户描述中提取部件和故障关键词作为 component_description/fault_description；
...
→ graph_query_diagnosis_path 查询诊断路径（用故障现象作 fault_description）
→ 综合分析后给出诊断结论
...
调用 knowledge_retrieval 时必须将图片URL通过 image_urls 参数传入，启用图文混合检索。
同时结合图片内容和文本描述进行综合分析。
"""
