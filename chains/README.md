...
├── __init__.py
├── README.md
└── orchestrator.py    # IntentionType → AgentMode 映射函数
```
...
map_intention_to_mode(IntentionType.QUERY_KNOWLEDGE)  # → AgentMode.RETRIEVAL
map_intention_to_mode(IntentionType.TROUBLESHOOT)     # → AgentMode.DIAGNOSIS
map_intention_to_mode(IntentionType.SEEK_GUIDANCE)    # → AgentMode.GUIDANCE
map_intention_to_mode(IntentionType.SUBMIT_CASE)      # → AgentMode.RETRIEVAL
map_intention_to_mode(IntentionType.GENERAL_CHAT)     # → AgentMode.CHAT
```