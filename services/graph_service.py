...
class GraphService:
    ...
    def create_relation(
        ...
        对应 Java 端 RelationServiceImpl.create()。

        Args:
            source_id: 源节点ID
            target_id: 目标节点ID
            relation_type: 关系类型，取值：
                - DEVICE_OWNS_COMPONENT: Device -> Component
                - DEVICE_HAS_FAULT: Device -> Fault
                - COMPONENT_CAUSES_FAULT: Component -> Fault
                - FAULT_HAS_SOLUTION: Fault -> Solution
                - CASE_RECORD_RECORDED_FAULT: CaseRecord -> Fault

        Returns:
            是否创建成功（两端节点都存在时返回 True）
        """
        relation_map = {
            "DEVICE_OWNS_COMPONENT": ("Device", "Component", "OWNS"),
            "DEVICE_HAS_FAULT": ("Device", "Fault", "HAS_FAULT"),
            "COMPONENT_CAUSES_FAULT": ("Component", "Fault", "CAUSES"),
            "FAULT_HAS_SOLUTION": ("Fault", "Solution", "HAS_SOLUTION"),
            "CASE_RECORD_RECORDED_FAULT": ("CaseRecord", "Fault", "RECORDED")
        }

        if relation_type not in relation_map:
            raise ValueError(f"未知的 relation_type: {relation_type}")

        source_label, target_label, rel_type = relation_map[relation_type]
        ...
        result = self._execute_single(cypher, {
            "sourceId": source_id,
            "targetId": target_id
        })
    ...
    def create_device(
        ...
        return result["id"] if result else device_id
    ...
    def create_component(
        ...
        return result["id"] if result else component_id
    ...
    def create_fault(
        ...
        Args:
            ...
            code: 故障编码（如 F001）
            description: 故障描述
            severity: 严重程度（轻微/一般/严重/致命）
            category: 故障类别（机械/电气/软件/其他）
        ...
        return result["id"] if result else fault_id
    ...
    def create_solution(
        ...
        result = self._execute_single(cypher, {
            ...
            "title": title,
            "description": description,
            "toolsRequired": tools_required,
            "estimatedTime": estimated_time,
            "difficulty": difficulty,
            "verified": verified
        })
        return result["id"] if result else solution_id
